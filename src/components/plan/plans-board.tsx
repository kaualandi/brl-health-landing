    ...
    if (isCurrent) {
      return (
        <Button
          disabled
          size="lg"
          className="mt-auto h-11 w-full cursor-default border border-white/15 bg-white/5 text-sm text-foreground disabled:opacity-100"
        >
          {plan.id === "free"
    ...
    if (plan.id === "free") {
      if (isAuthenticated) {
        return (
          <Button
            disabled
            size="lg"
            className="mt-auto h-11 w-full cursor-default border border-white/10 bg-transparent text-sm text-muted-foreground disabled:opacity-100"
          >
      ...
      return (
        <Button
          ...
          variant="outline"
          nativeButton={false}
          className="mt-auto h-11 w-full border border-white/15 bg-white/5 text-sm hover:bg-white/10"
      ...
      <div className="mt-14 grid items-stretch gap-6 md:grid-cols-2 lg:grid-cols-3">
        {Array.from({ length: 3 }).map((_, i) => (
          <div
            key={i}
            aria-hidden
            className="h-120 animate-pulse rounded-2xl border border-white/5 bg-brl-card"
          />
        ))}
      </div>
    );
